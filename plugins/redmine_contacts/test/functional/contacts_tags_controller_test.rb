...
class ContactsTagsControllerTest < ActionController::TestCase
  ...
  def test_should_get_api_index
    @request.session[:user_id] = 1
    compatible_request :get, :index, format: :json
  ...
  def test_should_get_edit
    ...
    assert_equal Redmineup::Tag.find(1).name, assigned_tag
  end

  def test_should_put_update
    @request.session[:user_id] = 1
    tag1 = Redmineup::Tag.find(1)
