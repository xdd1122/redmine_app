...
class NotesControllerTest < ActionController::TestCase
  ...
  def test_should_post_add_note_to_contact
    ...
    note_date = Date.today.to_date
    note_time = Time.now.strftime("%H:%M")
    assert_difference 'Note.count' do
      compatible_request :post, :create, :project_id => 1,
                                         :note => { subject: 'Note subject',
                                                    content: 'Note *content*',
                                                    type_id: 2,
                                                    created_on: "#{note_date} #{note_time}" },
    ...
    assert_equal 2, note.type_id
    assert_equal note_date, note.created_on.to_date
    assert_equal note_time, note.created_on.strftime("%H:%M")
  end
