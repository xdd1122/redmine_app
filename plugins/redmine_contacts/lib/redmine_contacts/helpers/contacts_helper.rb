  ...
  module Helper
    module CrmCalendarHelper
      ...
      def select_contact_tag(name, contacts, options = {})
        ...
        if options[:add_contact] && @project.try(:persisted?)
          if authorize_for('contacts', 'new')
            s << link_to(
              sprite_icon('add', css_class: 'icon-cf-value-add') || image_tag('add.png', style: 'vertical-align: middle; margin-left: 5px;'),
      ...
      def avatar_to(obj, options = {})
        ...
        obj_icon = obj.is_a?(Contact) ? (obj.is_company ? 'company.svg' : 'person.svg') : (obj.is_a?(Deal) ? 'deal.svg' : 'unknown.png')
      ...
      def note_type_icon(note)
        ...
        case note.type_id
        when 0
          note_type_tag = content_tag('span', sprite_icon('email'), :class => 'icon icon-email', :title => l(:label_crm_note_type_email))
        when 1
          note_type_tag = content_tag('span', sprite_icon('phone', plugin: :redmine_contacts), :class => 'icon icon-call', :title => l(:label_crm_note_type_call))
        when 2
          note_type_tag = content_tag('span', sprite_icon('meeting', plugin: :redmine_contacts), :class => 'icon icon-meeting', :title => l(:label_crm_note_type_meeting))
        end
